...
func ProcessUnique(_delta: float) -> void:
	## Calculate if Overheated Beforehand
	if !overheated && curr_overheat > max_overheat:
		overheated = true
	elif overheated && curr_overheat <= 0:
		overheated = false
	## if firing:
	if Input.is_action_pressed("left_click") && !overheated:
			...
			## Minigun Handle: Always Rotate Towards Mouse, Never care about RANGE (always ready_to_fire = true)
		...
		if (curr_overheat > 0):
			if overheated:
				## if overheated, regen slower (but barely)
				curr_overheat -= (_delta * 0.9)
			else:
				curr_overheat -= _delta
...
static func get_level_upgrades(itemdata: ItemData) -> Array[ItemData.LevelUpgrade]:
	...
	arr.append(get_stat_upgrade(StatsResource.DAMAGE, randf_range(0.2, 0.8), itemdata.level, ItemData.get_weighted_rarity(itemdata.level), 0.01, 0))
	arr.append(get_stat_upgrade(StatsResource.ATTACKSPEED, randf_range(0.6, 1.5), itemdata.level, ItemData.get_weighted_rarity(itemdata.level), 0.01, 0))
	return arr
## Returns a randomized stat object, using the given itemdata's variables like rarity
static func randomize_stats(itemdata: ItemData) -> StatsResource:
	var newstats: StatsResource = super(itemdata)
	set_stat_randomize(newstats, StatsResource.DAMAGE, randf_range(-0.3, 0.6), 0.01, 0)
	set_stat_randomize(newstats, StatsResource.ATTACKSPEED, randf_range(-0.3, 1.7), 0.01, 0)
	return newstats
