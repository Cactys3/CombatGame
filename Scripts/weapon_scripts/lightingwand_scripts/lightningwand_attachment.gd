...
static func get_level_upgrades(itemdata: ItemData) -> Array[ItemData.LevelUpgrade]:
	...
	arr.append(get_stat_upgrade(StatsResource.DAMAGE, randf_range(0.2, 1.2), itemdata.level, ItemData.get_weighted_rarity(itemdata.level), 0.01, 0))
	arr.append(get_stat_upgrade(StatsResource.ATTACKSPEED, randf_range(0.2, 2), itemdata.level, ItemData.get_weighted_rarity(itemdata.level), 0.01, 0))
	return arr
