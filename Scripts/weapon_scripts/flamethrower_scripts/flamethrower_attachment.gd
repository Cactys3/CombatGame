...
static func get_level_upgrades(itemdata: ItemData) -> Array[ItemData.LevelUpgrade]:
	...
	arr.append(get_stat_upgrade(StatsResource.BUILDUP, randf_range(1, 1.9), itemdata.level, ItemData.get_weighted_rarity(itemdata.level), 0.01, 0))
	return arr
## Returns a randomized stat object, using the given itemdata's variables like rarity
static func randomize_stats(itemdata: ItemData) -> StatsResource:
	var newstats: StatsResource = super(itemdata)
	set_stat_randomize(newstats, StatsResource.BUILDUP, randf_range(-0.6, 4), 0.01, 0)
	return newstats
